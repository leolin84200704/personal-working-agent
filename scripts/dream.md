...
- **`extract-failures.py` carries orphaned entries forward.** An entry whose STM has been archived is re-read out of the existing LTM file rather than dropped, and the run reports which ones it carried — note them in the dream log. It still refuses to write (exit 2) if entries would vanish even after that; treat a refusal as a defect to report, not something to clear with `--prune`.
